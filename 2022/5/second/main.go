...
func moveCargo(n string, src string, dst string, cargoState map[string][]string) {
	...
	fmt.Printf("qty:%v src:%v dst:%v\n", qty, src, dst)
	fmt.Println("before", cargoState[src], cargoState[dst])
	cargoState[dst] = append(cargoState[dst], cargoState[src][len(cargoState[src])-qty:]...) // add item from source stack to destination stack
	cargoState[src] = cargoState[src][:len(cargoState[src])-qty]                             // remove item from source stack
	fmt.Println("after", cargoState[src], cargoState[dst])
}
